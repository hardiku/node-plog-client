...
function PlogClient(options) {
  ...
  this._socket = null;
  this._lastMessageId = Math.floor(Math.random() * messageIdCeiling);
}

PlogClient.prototype.send = function (message) {
  var messageBuffer = new Buffer(message, 'utf-8'),
      messageId = this._nextMessageId(),
      messageChecksum = checksum.compute(messageBuffer),
      chunks = this._chunkBuffer(messageBuffer, this.chunkSize);
...
PlogClient.prototype._nextMessageId = function () {
  ...
  this._lastMessageId %= messageIdCeiling;
...
PlogClient.prototype._chunkBuffer = function(buffer, chunkSize) {
  ...
  for (var offset = 0; offset < buffer.length; offset += chunkSize) {
    var sliceLength = Math.min(buffer.length - offset, chunkSize)
    chunks.push(buffer.slice(offset, sliceLength));
  }
  return chunks;
}
